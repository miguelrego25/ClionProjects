...
void parsearray (char *token,char **resto, char *demilit,Tipoval *alfabeto,SPointer mini){
    ...
    for (token = __strtok_r(*resto, demilit,resto); strcmp(token,"]") != 0;token = __strtok_r(NULL,demilit,resto) ){
...
void parser (char *line){
    ...
    char delimit[8] = " \n\t";
    char *token;
    for(token = __strtok_r(line, delimit,&resto); token != NULL;token = __strtok_r(NULL,delimit,&resto) ){
        printf("token : %s\n", token);
        if (strstr("[",token) != NULL){
